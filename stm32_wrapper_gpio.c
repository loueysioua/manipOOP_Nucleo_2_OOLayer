...
{
	int pinID = (pin & 0x0F);
	return (get_gpioport(pin)->ODR & (1<<pinID));
}
	...
	void gpio_write(PinName pin, int value)
{
	int pinID = (pin & 0x0F);
	if(value == 0){
		get_gpioport(pin)->BRR |= 1<<pinID;
	}
	else {
		get_gpioport(pin)->BSRR |= 1<<pinID;
	}
}
...
{	
	switch(mode) {
		case PPull: return GPIO_Mode_Out_PP;
		case ODrain: return GPIO_Mode_Out_OD; 
		case AF_PPull: return GPIO_Mode_AF_PP;
		case AF_ODrain: return GPIO_Mode_AF_OD;
...
{	
	switch (speed) {
		case LowSpeed: return GPIO_Speed_2MHz; 
		case MediumSpeed: return GPIO_Speed_10MHz;
		case HighSpeed: return GPIO_Speed_50MHz;
...
static void _gpio_init_out(PinName pin, PinOutMode mode, PinSpeed speed)
{
	...
	GPIO_InitStruct.GPIO_Speed = (GPIOSpeed_TypeDef)get_gpio_out_speed(speed);
